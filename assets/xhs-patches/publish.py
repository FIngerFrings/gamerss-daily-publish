    ...
    while time.monotonic() < deadline:
        ...
        found = page.evaluate(
            ...
            (() => {{
                ...
                // 策略1: 先找精确匹配的 creator-tab 或其子节点
                const tabs = Array.from(document.querySelectorAll({json.dumps(CREATOR_TAB)}));
                for (const tab of tabs) {{
                    const titleSpan = tab.querySelector('span.title');
                    const tabText = titleSpan ? titleSpan.textContent.trim() : tab.textContent.trim();
                    if (tabText !== {json.dumps(tab_name)}) continue;
                    if (!isVisible(tab)) continue;
                    if (forceClick(tab)) return 'clicked';
                }}

                // 策略2: 查找覆盖层按钮或任意文本节点，向上找到可点击容器
                const allElements = Array.from(document.querySelectorAll('*'));
                for (const el of allElements) {{
                    const text = el.textContent ? el.textContent.trim() : '';
                    if (text !== {json.dumps(tab_name)}) continue;
                    const candidate = el.closest('div.creator-tab, [data-hp-kind], button, div, span') || el;
                    if (!isVisible(candidate) && !isVisible(el)) continue;
                    if (forceClick(candidate) || forceClick(el)) return 'clicked';
                }}
